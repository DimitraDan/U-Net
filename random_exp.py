...
def collate_fn(batch):
    ...
    return {'image': images_tensor, 'mask': masks_tensor}
...
def predict_and_save(net, input_path, output_path, scale_factor=0.5, out_threshold=0.5, device='cpu'):
    # Load the image
...
def train_model(
    ...
    for epoch in range(1, epochs + 1):
        ...
        average_dice_score = evaluate(model, val_loader, device, amp)
...
def train_evaluate_with_new_exp_images(
    name,
    model,
    exp_image_dir,
    exp_mask_dir,
    ...
    for i in range(step, num_exp_images, step):
        ...
        train_image_dir = f"/home/it21918/data/{name}/random/train_{num_train_images}_exp_{i}_image"
        train_mask_dir = f"/home/it21918/data/{name}/random/train_{num_train_images}_exp_{i}_mask"

        # Copy i random images and then delete them from exp folder
        copy_corresponding_random_images_and_delete(exp_image_dir, exp_mask_dir, train_image_dir, train_mask_dir, i)
        ...
        # Train the model 
        train_model(
            name=f"{name}/random/train_{num_train_images}_exp_{i}/",
            dir_checkpoint=f"/home/it21918/data/checkpoints/{name}/random/train_{num_train_images}_exp_{i}/",
        ...
        # Evaluate the model with validation images
        try:
...
exp_img_dir = "/home/it21918/data/exp3_image_random_1079"
exp_mask_dir = "/home/it21918/data/exp3_mask_random_1079"

train_evaluate_with_new_exp_images(
    name="exp3_10/100",
    model=model,
